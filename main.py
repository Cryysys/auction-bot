    ...
    try:
        ...
        if old_highest and old_highest != interaction.user:
            ...
            if bot.notification_prefs.get(pref_key, False):
                try:
                    await old_highest.send(f"You've been outbid for **{auction.item_name}**! New price: {format_price(bid_val, auction.currency_symbol)}")
                except: pass
...
async def quickbid(interaction: discord.Interaction):
    auction = bot.auctions.get(interaction.channel_id)
    if not auction:
        await interaction.response.send_message("No active auction here.", ephemeral=True)
        return
    ...
    try:
        ...
        master_embed.add_field(name="Current Bid", value=format_price(new_price, auction.currency_symbol), inline=True)
        master_embed.add_field(name="Min Increment", value=format_price(auction.min_increment, auction.currency_symbol), inline=True)
        master_embed.add_field(name="Highest Bidder", value=auction.highest_bidder.mention, inline=False)
        if auction.image_url:
            master_embed.set_thumbnail(url=auction.image_url)
        ...
        if old_highest and old_highest != interaction.user:
            ...
            if bot.notification_prefs.get(pref_key, False):
                try:
                    await old_highest.send(f"You've been outbid for **{auction.item_name}**! New price: {format_price(new_price, auction.currency_symbol)}")
                except: pass
